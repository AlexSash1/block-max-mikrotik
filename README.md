...
Набор правил для MikroTik RouterOS, блокирующих клиент MAX и сервисы определения ip, которые использует МАКС. 
...
- Добавляет статические DNS-записи (тип FWD) для следующих доменов, перенаправляя их в address-list `BlockMax`:
  - Сам клиент
    - api.oneme.ru
    - api-gost.oneme.ru
  - сервисы определения ip, которые использует MAX
    - api.ipify.org
...
```
## Примечания
- Переместите правило BlockMax после Fasttrack, обычно после forward defconf: accept established, related, untracked
- Для разрешения работы MAX, удалите записи с api.oneme.ru, api-gost.oneme.ru
